 ...
 * - groq    — Groq's OpenAI-compatible chat completions. On the free tier
 *             only qwen/qwen3.6-27b accepts image input (live-verified:
 *             openai/gpt-oss-* and the llama models on Groq are served
 *             text-only — the API rejects image_url parts with
 *             "content must be a string"). See README for free-plan limits.
...
const PROVIDER_DEFAULTS = {
  ...
  groq: {
    ...
    // qwen/qwen3.6-27b defaults to thinking mode; non-thinking returns a
    // plain description instead of a <think>…</think> block and does not
    // spend the completion budget on reasoning.
...
function apply(ctx, config) {
  ...
  const reasoningEffort = cfg.reasoningEffort ?? defaults.reasoningEffort ?? null
  const visionBaseURL =
    providerName === 'gemini'
