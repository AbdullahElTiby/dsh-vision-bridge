 ...
 *      images) is described by a Gemini vision model and replaced with a text
 *      block carrying the description. The session history and UI keep the
...
/** Gemini system instruction: produce a description a blind model can use. */
...
function apply(ctx, config) {
  ...
  async function geminiKey() {
    const resolved = await credentials.resolve(cfg.apiKeyRef)
    const key = resolved?.value
    if (!key && !keyMissingLogged) {
      keyMissingLogged = true
      log(
        `no "${cfg.apiKeyRef}" credential; images will be replaced with failure placeholders until one is set (env, ~/.dsh/.credentials.yaml, or .env)`,
      )
    }
    return key
  }
  ...
  /**
   * Describe one image (or several) with Gemini.
   * @param images - [{ data: base64 string, mediaType }]
   * @param question - optional user question to answer.
   * @param signal - caller cancellation.
   */
  async function geminiDescribe(images, question, signal) {
    const key = await geminiKey()
    if (!key) throw new Error(`missing ${cfg.apiKeyRef} credential`)
    const parts = []
    ...
    const timer = setTimeout(() => controller.abort(new Error('vision-bridge Gemini call timed out')), cfg.timeoutMs)
   ...
   * the user's question). One Gemini call per image, cached per attachment.
   */
    ...
    for (const image of images) {
      ...
      try {
        ...
        if (bytes.byteLength > cfg.maxImageBytes) {
          ...
          description = await geminiDescribe(
